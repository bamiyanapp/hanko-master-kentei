...
デプロイ（`.github/workflows/cd.yml`）は `main` ブランチへのpushをトリガーに直接デプロイする独自運用を維持する。frontend（GitHub Pages）はdev-standardsの共通複合action `deploy-github-pages`（タグ固定参照）を利用し、backend（Serverless Framework）はプロダクト固有の手順とする。dev-standardsの `reusable-cd.yml`（semantic-release・releaseブランチ運用が前提）は、本リポジトリでは採用しない。
